...
function module:Resolve(chosenChild, parentSize, parentPosition, parentRotation)
	-- ts kinda buns
-- local parentSize, parentPosition, parentRotation = self.Parent.RenderSize, self.Parent.RenderPosition, self.Parent.RenderRotation
	
	local currentPosition = UDim2.new(0, 0, 0, 0)
	for _, child in ipairs(self.Parent:GetChildren()) do
		if child:IsA("Frame") then
			if child == chosenChild then
				return child.Size:Calculate(parentSize), parentPosition + (currentPosition:Calculate(parentSize) * self.ListAxis), parentRotation
			end
