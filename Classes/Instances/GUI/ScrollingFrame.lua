...
module.new = function()
	...
	self.Maid:Add(Game:GetService("InputService").Scrolled:Connect(function(dir)
		if not self:MouseHovering() then return end
		task.spawn(function()
			for i = 1, 3 do
				self.CanvasPosition = self.CanvasPosition + Vector.new(0, -dir*10)
				task.wait()
			end
		end)
	end))
...
function module:Update(dt)
	...
	self.CanvasPosition = Vector.new(self.CanvasPosition.X, math.clamp(self.CanvasPosition.Y, 0, self.RenderCanvasSize.Y-self.RenderSize.Y))
